...
$("button").click(function() {  // clicking the button starts the game
  $("button").slideUp(); // slides up the button when game starts
...
function checkAnswer(currentLevel) { // a function that gets the parameter from userClickedPattern -1
  ...
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) { // checks if gamePattern[] and userClickedPattern[] are complete equal
    ...
    $("#level-title").text("GAME OVER, Press GAME button to Restart"); // changes the text of the h1
    $("button").slideDown();  // slides the button down when game over
